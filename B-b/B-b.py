    ...
    for key in uiKeys:
        ...
        try:
            u_feature = type_1_1[key[0]] + type_2_11[key[0]] + type_2_3[key[0]] + type_4_221[key[0]] + \
                        type_1_21[key[0]] + type_2_12[key[0]] + type_4_11[key[0]] + type_4_121[key]
        except KeyError:
        ...
        try:
            i_feature = type_1_1[key[1]] + type_2_11[key[1]] + type_1_3[key[1]] + type_2_2[key[1]] + type_4_11[key[1]] + \
                        type_4_13[key[1]] + type_4_23[key[1]] + type_4_31[key[1]]
        except KeyError:
            i_feature = [0] * 12 + [0] * 12 + [0] * 4 + [0] * 12 + [0] * 3 + [0] * 1 + [0] * 1 + [0] * 4
...
def make_time_picture(times):
    ...
    plt.bar(x=range(1, len(times) + 1), height=times, width=0.6, color="g")
...
def make_precise_picture(precises):
    plt.figure(figsize=(15, 8))
    plt.bar(x=range(1, len(precises) + 1), height=precises, width=0.6, color="g")
    plt.xlabel('classifier')
    plt.ylabel('precise (%)')
...
if __name__ == "__main__":
    ...
    uiDf = tradeDf.groupby(['vipno', 'pluno'])
    ...
    test_type_2_2_feature = get_type_2_2_feature(months)
